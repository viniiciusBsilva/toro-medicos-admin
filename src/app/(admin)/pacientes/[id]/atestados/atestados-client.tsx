...
type Props = {
  pacienteId: string;
  pacienteNome: string;
  atestados: DocumentoItem[];
};

export function AtestadosClient({ pacienteId, pacienteNome, atestados }: Props) {
  ...
  function handleDownload(doc: DocumentoItem) {
    if (doc.arquivo_url) {
      ...
      a.download = doc.nome_arquivo ?? "atestado.pdf";
      a.target = "_blank";
      a.click();
      return;
    }
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `atestado-${doc.id.slice(0, 8)}.pdf`;
    a.click();
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-4">
        ...
        <h2 className="text-xl font-semibold text-text-primary">Atestados</h2>
      </div>
                    ...
                    <div className="space-y-2 min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-2 text-text-primary">
                        ...
                        <span className="truncate">
                          {doc.nome_arquivo ?? (temDados ? "Atestado" : "Documento anexado")}
                        </span>
                      </div>
                    </div>
                    {temDados && (
